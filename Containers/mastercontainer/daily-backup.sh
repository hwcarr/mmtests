...
APACHE_PORT="$(docker inspect minimailboxes-aio-apache --format "{{.HostConfig.PortBindings}}" | grep -oP '[0-9]+' | head -1)"
while docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-apache$" && ! nc -z minimailboxes-aio-apache "$APACHE_PORT"; do
    echo "Waiting for apache to become available"
    sleep 30
done
while docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-watchtower$"; do
    echo "Waiting for watchtower to stop"
    sleep 30
done
while docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-borgbackup$"; do
...
if [ "$AUTOMATIC_UPDATES" = 1 ] && ! docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-watchtower$"; then
...
if [ "$DAILY_BACKUP" = 1 ] && ([ "$AUTOMATIC_UPDATES" = 1 ] || [ "$START_CONTAINERS" = 1 ]); then
    # Wait for the minimailboxes container to start and send if the backup was successful
    if ! docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-minimailboxes$"; then
        echo "Something seems to be wrong: MiniMailboxes should be started at this step."
    else
        while docker ps --format "{{.Names}}" | grep -q "^minimailboxes-aio-minimailboxes$" && ! nc -z minimailboxes-aio-minimailboxes 9000; do
            echo "Waiting for the MiniMailboxes container to start"
            sleep 30
            if [ "$(docker inspect minimailboxes-aio-minimailboxes --format "{{.State.Restarting}}")" = "true" ]; then
                echo "MiniMailboxes container restarting. Skipping this check!"
                break
            fi
        done
    fi
