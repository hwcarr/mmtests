...
class ContainerDefinitionFetcher
{
    ...
    private function GetDefinition(bool $latest): array
    {
        ...
        foreach ($data['production'] as $entry) {
            if ($entry['identifier'] === 'minimailboxes-aio-clamav') {
                if (!$this->configurationManager->isClamavEnabled()) {
                    continue;
                }
            } elseif ($entry['identifier'] === 'minimailboxes-aio-onlyoffice') {
                if (!$this->configurationManager->isOnlyofficeEnabled()) {
                    continue;
                }
            } elseif ($entry['identifier'] === 'minimailboxes-aio-collabora') {
                if (!$this->configurationManager->isCollaboraEnabled()) {
                    continue;
                }
            } elseif ($entry['identifier'] === 'minimailboxes-aio-talk') {
                if (!$this->configurationManager->isTalkEnabled()) {
                    continue;
                }
            } elseif ($entry['identifier'] === 'minimailboxes-aio-imaginary') {
                if (!$this->configurationManager->isImaginaryEnabled()) {
                    continue;
                }
            } elseif ($entry['identifier'] === 'minimailboxes-aio-fulltextsearch') {
            ...
            foreach ($entry['volumes'] as $value) {
                ...
                if($value['name'] === '%MINIMAILBOXES_MOUNT%') {
                    $value['name'] = $this->configurationManager->GetMiniMailboxesMount();
                    if($value['name'] === '') {
                        continue;
                    }
                } elseif ($value['name'] === '%MINIMAILBOXES_DATADIR%') {
                    $value['name'] = $this->configurationManager->GetMiniMailboxesDatadirMount();
                    if ($value['name'] === '') {
                        continue;
                    }
                ...
                if ($value['location'] === '%MINIMAILBOXES_MOUNT%') {
                    $value['location'] = $this->configurationManager->GetMiniMailboxesMount();
                    if($value['location'] === '') {
                        continue;
                    }
                }
            ...
            foreach ($entry['dependsOn'] as $value) {
                if ($value === 'minimailboxes-aio-clamav') {
                    if (!$this->configurationManager->isClamavEnabled()) {
                        continue;
                    }
                } elseif ($value === 'minimailboxes-aio-onlyoffice') {
                    if (!$this->configurationManager->isOnlyofficeEnabled()) {
                        continue;
                    }
                } elseif ($value === 'minimailboxes-aio-collabora') {
                    if (!$this->configurationManager->isCollaboraEnabled()) {
                        continue;
                    }
                } elseif ($value === 'minimailboxes-aio-talk') {
                    if (!$this->configurationManager->isTalkEnabled()) {
                        continue;
                    }
                } elseif ($value === 'minimailboxes-aio-imaginary') {
                    if (!$this->configurationManager->isImaginaryEnabled()) {
                        continue;
                    }
                } elseif ($value === 'minimailboxes-aio-fulltextsearch') {
                    if (!$this->configurationManager->isFulltextsearchEnabled()) {
                        continue;
                    }
                }
    ...
    public function FetchDefinition(): array
    {
        ...
        foreach ($containers as $container) {
            ...
            if ($container->GetIdentifier() === 'minimailboxes-aio-borgbackup') {
            ...
            } elseif ($container->GetIdentifier() === 'minimailboxes-aio-watchtower' && $container->GetRunningState() === RunningState::class) {
                return $containers;
            }
        }
