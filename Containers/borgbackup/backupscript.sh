...
VOLUME_DIRS="$(find /minimailboxes_aio_volumes -mindepth 1 -maxdepth 1 -type d)"
...
if [ "$BORG_MODE" = backup ] || [ "$BORG_MODE" = restore ]; then
    touch "/minimailboxes_aio_volumes/minimailboxes_aio_database_dump/backup-is-running"
fi
...
if [ "$BORG_MODE" = backup ]; then
    ...
    if ! [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json" ]; then
        echo "configuration.json not present. Cannot perform the backup!"
        exit 1
    elif ! [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_minimailboxes/config/config.php" ]; then
        echo "config.php is missing cannot perform backup"
        exit 1
    elif ! [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_database_dump/database-dump.sql" ]; then
    ...
    if [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_database_dump/export.failed" ]; then
        echo "Database export failed the last time. Most likely was the export time not high enough."
        echo "Cannot create a backup now."
        echo "Please report this to https://github.com/minimailboxes/all-in-one/issues. Thanks!"
        exit 1
    fi
    ...
    if ! [ -f "$BORG_BACKUP_DIRECTORY/config" ]; then
        ...
        if [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/borg.config" ]; then
        ...
        rm -f "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/borg.config"
        if ! cp "$BORG_BACKUP_DIRECTORY/config" "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/borg.config"; then
    ...
    get_start_time
    if ! borg create "${BORG_OPTS[@]}" "$BORG_BACKUP_DIRECTORY::$CURRENT_DATE-minimailboxes-aio" "/minimailboxes_aio_volumes/"; then
        echo "Deleting the failed backup archive..."
        borg delete --stats "$BORG_BACKUP_DIRECTORY::$CURRENT_DATE-minimailboxes-aio"
        echo "Backup failed!"
        if [ "$NEW_REPOSITORY" = 1 ]; then
            echo "Deleting borg.config file so that you can choose a different location for the backup."
            rm "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/borg.config"
        fi
        exit 1
    fi

    # Remove the update skip file because the backup was successful
    rm -f "/minimailboxes_aio_volumes/minimailboxes_aio_minimailboxes_data/skip.update"
    ...
    if ! borg prune --prefix '*_*-minimailboxes-aio' "${BORG_PRUNE_OPTS[@]}"; then
    ...
    if [ "$ADDITIONAL_DIRECTORIES_BACKUP" = 'yes' ]; then
        ...
        if [ -d "/host_mounts/" ]; then
            ...
            EXCLUDED_DIRECTORIES+=(var/lib/docker/volumes/minimailboxes_aio_backup_cache/_data)
            # Exclude target directory
            if [ -n "$BORGBACKUP_HOST_LOCATION" ] && [ "$BORGBACKUP_HOST_LOCATION" != "minimailboxes_aio_backupdir" ]; then
    ...
    if [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_minimailboxes_data/update.failed" ]; then
        echo "However a MiniMailboxes update failed. So reporting that the backup failed which will skip any update attempt the next time."
        echo "Please restore a backup from before the failed MiniMailboxes update attempt."
        exit 1
    fi
    exit 0
fi
...
if [ "$BORG_MODE" = restore ]; then
    ...
    if [ -n "$SELECTED_RESTORE_TIME" ]; then
        SELECTED_ARCHIVE="$(borg list "$BORG_BACKUP_DIRECTORY" | grep "minimailboxes-aio" | grep "$SELECTED_RESTORE_TIME" | awk -F " " '{print $1}' | head -1)"
    else
        SELECTED_ARCHIVE="$(borg list "$BORG_BACKUP_DIRECTORY" | grep "minimailboxes-aio" | awk -F " " '{print $1}' | sort -r | head -1)"
    fi
    ...
    if [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/additional_backup_directories" ]; then
        ADDITIONAL_BACKUP_DIRECTORIES="$(cat /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/additional_backup_directories)"
    fi

    # Save daily backup time
    if [ -f "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/daily_backup_time" ]; then
        DAILY_BACKUPTIME="$(cat /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/daily_backup_time)"
    fi

    # Restore everything except the configuration file
    if ! rsync --stats --archive --human-readable -vv --delete \
    --exclude "minimailboxes_aio_mastercontainer/session/"** \
    --exclude "minimailboxes_aio_mastercontainer/certs/"** \
    --exclude "minimailboxes_aio_mastercontainer/data/daily_backup_running" \
    --exclude "minimailboxes_aio_mastercontainer/data/session_date_file" \
    --exclude "minimailboxes_aio_mastercontainer/data/configuration.json" \
    /tmp/borg/minimailboxes_aio_volumes/ /minimailboxes_aio_volumes; then
    ...
    AIO_PASSWORD="$(jq '.password' /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"

    # Save current path
    BORG_LOCATION="$(jq '.borg_backup_host_location' /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"

    # Save current minimailboxes datadir
    if grep -q '"minimailboxes_datadir":' /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json; then
        MINIMAILBOXES_DATADIR="$(jq '.minimailboxes_datadir' /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"
    else
        MINIMAILBOXES_DATADIR='""'
    fi

    # Restore the configuration file
    if ! rsync --archive --human-readable -vv \
    /tmp/borg/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json \
    /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json; then
    ...
    CONTENTS="$(jq '."backup-mode" = "restore"' /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"
    echo -E "${CONTENTS}" > /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json

    # Reset the backup path to the currently used one
    CONTENTS="$(jq ".borg_backup_host_location = $BORG_LOCATION" /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"
    echo -E "${CONTENTS}" > /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json

    # Reset the AIO password to the currently used one
    CONTENTS="$(jq ".password = $AIO_PASSWORD" /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"
    echo -E "${CONTENTS}" > /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json

    # Reset the datadir to the one that was used for the restore
    CONTENTS="$(jq ".minimailboxes_datadir = $MINIMAILBOXES_DATADIR" /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json)"
    echo -E "${CONTENTS}" > /minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/configuration.json

    # Reset the additional backup directories
    if [ -n "$ADDITIONAL_BACKUP_DIRECTORIES" ]; then
        echo "$ADDITIONAL_BACKUP_DIRECTORIES" > "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/additional_backup_directories"
        chown 33:0 "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/additional_backup_directories"
        chmod 770 "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/additional_backup_directories"
    fi

    # Reset the additional backup directories
    if [ -n "$DAILY_BACKUPTIME" ]; then
        echo "$DAILY_BACKUPTIME" > "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/daily_backup_time"
        chown 33:0 "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/daily_backup_time"
        chmod 770 "/minimailboxes_aio_volumes/minimailboxes_aio_mastercontainer/data/daily_backup_time"
    fi
    ...
    # Add file to MiniMailboxes container so that it performs a fingerprint update the next time
    touch "/minimailboxes_aio_volumes/minimailboxes_aio_minimailboxes_data/fingerprint.update"
    chmod 777 "/minimailboxes_aio_volumes/minimailboxes_aio_minimailboxes_data/fingerprint.update"
fi
