...
class DockerActionManager
{
    ...
    public function CreateVolumes(Container $container): void
    {
        ...
        foreach($container->GetVolumes()->GetVolumes() as $volume) {
            ...
            if ($volume->name === 'minimailboxes_aio_minimailboxes_datadir' || $volume->name === 'minimailboxes_aio_backupdir') {
                return;
            }
    ...
    public function CreateContainer(Container $container) : void {
        ...
        foreach($envs as $key => $env) {
            ...
            if(preg_match($patterns[0], $env, $out) === 1) {
                ...
                } elseif ($out[1] === 'MINIMAILBOXES_MOUNT') {
                    $replacements[1] = $this->configurationManager->GetMiniMailboxesMount();
                ...
                } elseif ($out[1] === 'UPDATE_MINIMAILBOXES_APPS') {
                ...
                } elseif ($out[1] === 'MINIMAILBOXES_UPLOAD_LIMIT') {
                    $replacements[1] = $this->configurationManager->GetMiniMailboxesUploadLimit();
                } elseif ($out[1] === 'MINIMAILBOXES_MAX_TIME') {
                    $replacements[1] = $this->configurationManager->GetMiniMailboxesMaxTime();
    ...
    public function GetCurrentChannel() : string {
        ...
        $containerName = 'minimailboxes-aio-mastercontainer';
    ...
    {
        $imageName = 'minimailboxes/all-in-one';
        $containerName = 'minimailboxes-aio-mastercontainer';
    ...
    public function DisconnectContainerFromNetwork(Container $container) : void
    {
        ...
        $url = $this->BuildApiUrl(
            sprintf('networks/%s/disconnect', 'minimailboxes-aio')
        );
    ...
    private function ConnectContainerIdToNetwork(string $id) : void
    {
        ...
        $url = $this->BuildApiUrl(
            sprintf('networks/%s/connect', 'minimailboxes-aio')
        );
        try {
            $this->guzzleClient->request(
                'POST',
    ...
    {
        $this->ConnectContainerIdToNetwork('minimailboxes-aio-mastercontainer');
    }
    ...
    {
        $containerName = 'minimailboxes-aio-database';
        $url = $this->BuildApiUrl(sprintf('containers/%s/json', urlencode($containerName)));
        try {
            $response = $this->guzzleClient->get($url);
        } catch (RequestException $e) {
    ...
    public function isBackupContainerRunning() : bool {
        $id = 'minimailboxes-aio-borgbackup';
