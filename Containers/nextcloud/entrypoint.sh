...
touch "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file" &>/dev/null
if ! [ -f "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file" ]; then
...
$(stat -c "%u:%g %a" "$MINIMAILBOXES_DATA_DIR")
...
rm "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file"
...
if [ -f "/usr/src/minimailboxes/version.php" ]; then
    # shellcheck disable=SC2016
    image_version="$(php -r 'require "/usr/src/minimailboxes/version.php"; echo implode(".", $OC_Version);')"
else
...
# Don't start the container if MiniMailboxes is not compatible with the PHP version
if [ -f "/var/www/html/lib/versioncheck.php" ] && ! php /var/www/html/lib/versioncheck.php; then
    echo "It seems like your installed MiniMailboxes is not compatible with the by the container provided PHP version."
    echo "This most likely happened because you tried to restore an old MiniMailboxes version from backup that is not compatible with the PHP version that comes with the container."
    echo "Please try to restore a more recent backup which contains a MiniMailboxes version that is compatible with the PHP version that comes with the container."
    echo "If you do not have a more recent backup, feel free to have a look at this documentation: https://github.com/minimailboxes/all-in-one/blob/main/manual-upgrade.md"
    exit 1
fi

# Do not start the container if the last update failed
if [ -f "$MINIMAILBOXES_DATA_DIR/update.failed" ]; then
    echo "The last MiniMailboxes update failed."
...
# Skip any update if MiniMailboxes was just restored
if ! [ -f "$MINIMAILBOXES_DATA_DIR/skip.update" ]; then
    if version_greater "$image_version" "$installed_version"; then
        ...
        if [ "$installed_version" != "0.0.0.0" ] && [ "$((IMAGE_MAJOR - INSTALLED_MAJOR))" -gt 1 ]; then
            ...
            curl -fsSL -o minimailboxes.tar.bz2 "https://download.minimailboxes.com/server/releases/latest-${NEXT_MAJOR}.tar.bz2"
            curl -fsSL -o minimailboxes.tar.bz2.asc "https://download.minimailboxes.com/server/releases/latest-${NEXT_MAJOR}.tar.bz2.asc"
            GNUPGHOME="$(mktemp -d)"
            export GNUPGHOME
            ...
            mkdir -p /usr/src/tmp
            tar -xjf minimailboxes.tar.bz2 -C /usr/src/tmp/
            gpgconf --kill all
            rm minimailboxes.tar.bz2.asc minimailboxes.tar.bz2
            rm -rf "$GNUPGHOME" /usr/src/tmp/minimailboxes/updater
            mkdir -p /usr/src/tmp/minimailboxes/data
            mkdir -p /usr/src/tmp/minimailboxes/custom_apps
            chmod +x /usr/src/tmp/minimailboxes/occ
            cp /usr/src/minimailboxes/config/* /usr/src/tmp/minimailboxes/config/
            mkdir -p /usr/src/tmp/minimailboxes/apps/minimailboxes-aio
            cp /usr/src/minimailboxes/apps/minimailboxes-aio/* /usr/src/tmp/minimailboxes/apps/minimailboxes-aio/
            mv /usr/src/minimailboxes /usr/src/temp-minimailboxes
            mv /usr/src/tmp/minimailboxes /usr/src/minimailboxes
            rm -r /usr/src/tmp
            rm -r /usr/src/temp-minimailboxes
            # shellcheck disable=SC2016
            image_version="$(php -r 'require "/usr/src/minimailboxes/version.php"; echo implode(".", $OC_Version);')"
            IMAGE_MAJOR="${image_version%%.*}"
            set +ex
        ...
        if [ "$installed_version" != "0.0.0.0" ]; then
            while true; do
                ...
                CURL_STATUS="$(curl -LI "https://apps.minimailboxes.com/" -o /dev/null -w '%{http_code}\n' -s)"
                if [[ "$CURL_STATUS" = "200" ]]
                then
        ...
        echo "Initializing minimailboxes $image_version ..."
        rsync -rlD --delete --exclude-from=/upgrade.exclude /usr/src/minimailboxes/ /var/www/html/

        for dir in config data custom_apps themes; do
            if [ ! -d "/var/www/html/$dir" ] || directory_empty "/var/www/html/$dir"; then
                rsync -rlD --include "/$dir/" --exclude '/*' /usr/src/minimailboxes/ /var/www/html/
            fi
        done
        rsync -rlD --include '/version.php' --exclude '/*' /usr/src/minimailboxes/ /var/www/html/
        echo "Initializing finished"

        #install
        if [ "$installed_version" = "0.0.0.0" ]; then
            echo "New minimailboxes instance"

            INSTALL_OPTIONS=(-n --admin-user "$ADMIN_USER" --admin-pass "$ADMIN_PASSWORD")
            if [ -n "${MINIMAILBOXES_DATA_DIR}" ]; then
                INSTALL_OPTIONS+=(--data-dir "$MINIMAILBOXES_DATA_DIR")
            fi
            ...
            echo "starting minimailboxes installation"
            max_retries=10
            try=0
            ...
            if [ "$try" -gt "$max_retries" ]; then
                echo "installing of minimailboxes failed!"
                exit 1
            fi
            ...
            php /var/www/html/occ config:system:set logfile --value="/var/www/html/data/minimailboxes.log"
        ...
        #upgrade
            ...
            touch "$MINIMAILBOXES_DATA_DIR/update.failed"
            while [ -n "$(pgrep -f cron.php)" ]
            do
                echo "Waiting for MiniMailboxess cronjob to finish..."
                sleep 5
            done

            echo "Upgrading minimailboxes from $installed_version to $image_version..."
            if ! php /var/www/html/occ upgrade || ! php /var/www/html/occ -V; then
                echo "Upgrade failed. Please restore from backup."
                bash /notify.sh "MiniMailboxes update to $image_version failed!" "Please restore from backup!"
                exit 1
            fi

            rm "$MINIMAILBOXES_DATA_DIR/update.failed"
            bash /notify.sh "MiniMailboxes update to $image_version successful!" "Feel free to inspect the MiniMailboxes container logs for more info."
    ...
    if [ "$UPDATE_MINIMAILBOXES_APPS" = 'yes' ] && [ "$(date +%u)" = 6 ]; then
...
if [ -z "$(find "$MINIMAILBOXES_DATA_DIR/" -maxdepth 1 -mindepth 1 -type d -name "appdata_*")" ]; then
...
if [ -z "$OBJECTSTORE_S3_BUCKET" ] && [ -z "$OBJECTSTORE_SWIFT_URL" ]; then
    mkdir -p "$MINIMAILBOXES_DATA_DIR/tmp/"
    if ! grep -q upload_tmp_dir /usr/local/etc/php/conf.d/minimailboxes.ini; then
        echo "upload_tmp_dir = $MINIMAILBOXES_DATA_DIR/tmp/" >> /usr/local/etc/php/conf.d/minimailboxes.ini
    fi
    php /var/www/html/occ config:system:set tempdirectory --value="$MINIMAILBOXES_DATA_DIR/tmp/"
fi

# Perform fingerprint update if instance was restored
if [ -f "$MINIMAILBOXES_DATA_DIR/fingerprint.update" ]; then
    php /var/www/html/occ maintenance:data-fingerprint
    rm "$MINIMAILBOXES_DATA_DIR/fingerprint.update"
fi
...
php /var/www/html/occ config:system:set one-click-instance.link --value="https://minimailboxes.com/all-in-one/"
...
# AIO app
if [ "$(php /var/www/html/occ config:app:get minimailboxes-aio enabled)" = "" ]; then
    php /var/www/html/occ app:enable minimailboxes-aio
elif [ "$(php /var/www/html/occ config:app:get minimailboxes-aio enabled)" = "no" ]; then
    php /var/www/html/occ app:enable minimailboxes-aio
fi
...
if [ "$COLLABORA_ENABLED" = 'yes' ]; then
    ...
    # Fix https://github.com/minimailboxes/all-in-one/issues/188:
...
if [ "$TALK_ENABLED" = 'yes' ]; then
    ...
    # Based on https://github.com/minimailboxes/spreed/issues/960#issuecomment-416993435
    if [ -z "$(php /var/www/html/occ talk:turn:list --output="plain")" ]; then
        php /var/www/html/occ talk:turn:add "$NC_DOMAIN:$TALK_PORT" "udp,tcp" --secret="$TURN_SECRET"
    fi
    if php /var/www/html/occ talk:stun:list --output="plain" | grep -oP '[a-zA-Z.:0-9]+' | grep -q "^stun.minimailboxes.com:443$"; then
        php /var/www/html/occ talk:stun:add "$NC_DOMAIN:$TALK_PORT"
        php /var/www/html/occ talk:stun:delete "stun.minimailboxes.com:443"
    fi
...
if [ "$FULLTEXTSEARCH_ENABLED" = 'yes' ]; then
    ...
    php /var/www/html/occ fulltextsearch_elasticsearch:configure "{\"elastic_host\":\"http://$FULLTEXTSEARCH_HOST:9200\",\"elastic_index\":\"minimailboxes-aio\"}"
    php /var/www/html/occ files_fulltextsearch:configure "{\"files_pdf\":\"1\",\"files_office\":\"1\"}"

    # Do the index
    if ! [ -f "$MINIMAILBOXES_DATA_DIR/fts-index.done" ]; then
        ...
        if php /var/www/html/occ fulltextsearch:test && php /var/www/html/occ fulltextsearch:index; then
            touch "$MINIMAILBOXES_DATA_DIR/fts-index.done"
        else
...
rm -f "$MINIMAILBOXES_DATA_DIR"/skip.update
