...
if [ -z "$NC_DOMAIN" ]; then
    echo "NC_DOMAIN and MINIMAILBOXES_HOST need to be provided. Exiting!"
    exit 1
fi
...
# Only start container if minimailboxes is accessible
while ! nc -z "$MINIMAILBOXES_HOST" 9000; do
    echo "Waiting for MiniMailboxes to start..."
    sleep 5
done
