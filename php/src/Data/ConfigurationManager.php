...
class ConfigurationManager
{
    ...
    public function SetDomain(string $domain) : void {
        ...
        if (!$this->shouldDomainValidationBeSkipped()) {
            ...
            if ($response !== $instanceID) {
                ...
                throw new InvalidSettingConfigurationException("Domain does not point to this server or the reverse proxy is not configured correctly. See the mastercontainer logs for more details. ('sudo docker logs -f minimailboxes-aio-mastercontainer')");
            }
        }
    ...
    public function SetBorgBackupHostLocation(string $location) : void {
        ...
        if (str_starts_with($location, '/') && !str_ends_with($location, '/')) {
            $isValidPath = true;
        } elseif ($location === 'minimailboxes_aio_backupdir') {
            $isValidPath = true;
        }
    ...
    public function GetMiniMailboxesMount() : string {
        $envVariableName = 'MINIMAILBOXES_MOUNT';
        $configName = 'minimailboxes_mount';
        $defaultValue = '';
        return $this->GetEnvironmentalVariableOrConfig($envVariableName, $configName, $defaultValue);
    }

    public function GetMiniMailboxesDatadirMount() : string {
        $envVariableName = 'MINIMAILBOXES_DATADIR';
        $configName = 'minimailboxes_datadir';
        $defaultValue = 'minimailboxes_aio_minimailboxes_data';
        return $this->GetEnvironmentalVariableOrConfig($envVariableName, $configName, $defaultValue);
    }

    public function GetMiniMailboxesUploadLimit() : string {
        $envVariableName = 'MINIMAILBOXES_UPLOAD_LIMIT';
        $configName = 'minimailboxes_upload_limit';
        $defaultValue = '10G';
        return $this->GetEnvironmentalVariableOrConfig($envVariableName, $configName, $defaultValue);
    }

    public function GetApacheMaxSize() : int {
        $uploadLimit = (int)rtrim($this->GetMiniMailboxesUploadLimit(), 'G');
        return $uploadLimit * 1024 * 1024 * 1024;
    }

    public function GetMiniMailboxesMaxTime() : string {
        $envVariableName = 'MINIMAILBOXES_MAX_TIME';
        $configName = 'minimailboxes_max_time';
        $defaultValue = '3600';
        return $this->GetEnvironmentalVariableOrConfig($envVariableName, $configName, $defaultValue);
    }
