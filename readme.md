# MiniMailboxes All In One
MiniMailboxes AIO stands for MiniMailboxes All In One and provides easy deployment and maintenance with most features included in this one MiniMailboxes instance. 

Included are:
- MiniMailboxes
- MiniMailboxes Office
- High performance backend for MiniMailboxes Files
- High performance backend for MiniMailboxes Talk
...
2. Run the command below in order to start the container:<br><br>
    ...
    minimailboxes/all-in-one:latest
    ```
    <details>
    ...
    --volume minimailboxes_aio_mastercontainer:/mnt/docker-aio-config \
    --volume /var/run/docker.sock:/var/run/docker.sock:ro \
    minimailboxes/all-in-one:latest-arm64
    ```

    </details>

3. After the initial startup, you should be able to open the MiniMailboxes AIO Interface now on port 8080 of this server.<br>
E.g. `https://ip.address.of.this.server:8080`<br><br>
If your firewall/router has port 80 and 8443 open and you point a domain to your server, you can get a valid certificate automatically by opening the MiniMailboxes AIO Interface via:<br>
`https://your-domain-that-points-to-this-server.tld:8443`
4. Please do not forget to open port `3478/TCP` and `3478/UDP` in your firewall/router for the Talk container!

## FAQ
### How does it work?
MiniMailboxes AIO is inspired by projects like Portainer that manage the docker daemon by talking to it through the docker socket directly. This concept allows a user to install only one container with a single command that does the heavy lifting of creating and managing all containers that are needed in order to provide a MiniMailboxes installation with most features included. It also makes updating a breeze and is not bound to the host system (and its slow updates) anymore as everything is in containers. Additionally, it is very easy to handle from a user perspective because a simple interface for managing your MiniMailboxes AIO installation is provided.

### Are reverse proxies supported?
Yes. Please refer to the following documentation on this: [reverse-proxy.md](https://github.com/minimailboxes/all-in-one/blob/main/reverse-proxy.md)
...
- `80/TCP`: redirects to MiniMailboxes (is used for getting the certificate via ACME http-challenge for the Mastercontainer)
...
**Please note:** In order to make the built-in backup solution able to back up to the host system, you need to create a volume with the name `minimailboxes_aio_backupdir` beforehand:
```
docker volume create ^
--driver local ^
--name minimailboxes_aio_backupdir ^
...
Also, you may be interested in adjusting MiniMailboxes's Datadir to store the files on the host system. See [this documentation](https://github.com/minimailboxes/all-in-one#how-to-change-the-default-location-of-minimailboxess-datadir) on how to do it.
...
### How to run MiniMailboxes behind a Cloudflare Argo Tunnel?
...
### How to get MiniMailboxes running using the ACME DNS-challenge?
...
### How to run MiniMailboxes locally?
If you do not want to open MiniMailboxes to the public internet, you may have a look at the following documentation how to set it up locally: [local-instance.md](./local-instance.md)

### Are self-signed certificates supported for MiniMailboxes?
No and they will not be. If you want to run it locally, without opening MiniMailboxes to the public internet, please have a look at the [local instance documentation](./local-instance.md).

### Can I use an ip-address for MiniMailboxes instead of a domain?
...
### Are other ports than then default 443 for MiniMailboxes supported?
No and they will not be. Please use a dedicated domain for MiniMailboxes and set it up correctly by following the [reverse proxy documentation](./reverse-proxy.md). If port 443 and/or 80 is blocked for you, you may use the ACME DNS-challenge or a Cloudflare Argo Tunnel.

### Can I run MiniMailboxes in a subdirectory on my domain?
No and it will not be added. Please use a dedicated domain for MiniMailboxes and set it up correctly by following the [reverse proxy documentation](./reverse-proxy.md).

### How can I access MiniMailboxes locally?
The recommended way is to set up a local dns-server like a pi-hole and set up a custom dns-record for that domain that points to the internal ip-adddress of your server that runs MiniMailboxes AIO. Below are some guides:
...
Simply run the following: `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ your-command`. Of course `your-command` needs to be exchanged with the command that you want to run.

### How to resolve `Security & setup warnings displays the "missing default phone region" after initial install`?
Simply run the following command: `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ config:system:set default_phone_region --value="yourvalue"`. Of course you need to modify `yourvalue` based on your location. Examples are `DE`, `EN` and `GB`. See this list for more codes: https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements
...
MiniMailboxes features a built-in bruteforce protection which may get triggered and will block an ip-address or disable a user. You can unblock an ip-address by running `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ security:bruteforce:reset <ip-address>` and enable a disabled user by running `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ user:enable <name of user>`. See https://docs.minimailboxes.com/server/latest/admin_manual/configuration_server/occ_command.html#security for further information.

### Update policy
This project values stability over new features. That means that when a new major MiniMailboxes update gets introduced, we will wait at least until the first patch release, e.g. `24.0.1` is out before upgrading to it. Also we will wait with the upgrade until all important apps are compatible with the new major version. Minor or patch releases for MiniMailboxes and all dependencies as well as all containers will be updated to new versions as soon as possible but we try to give all updates first a good test round before pushing them. That means that it can take around 2 weeks before new updates reach the `latest` channel. If you want to help testing, you can switch to the `beta` channel by following [this documentation](#how-to-switch-the-channel) which will also give you the updates earlier.

### How to switch the channel?
You can switch to a different channel like e.g. the beta channel or from the beta channel back to the latest channel by stopping the mastercontainer, removing it (no data will be lost) and recreating the container using the same command that you used initially to create the mastercontainer. For the beta channel on x64 you need to change the last line `minimailboxes/all-in-one:latest` to `minimailboxes/all-in-one:beta` and vice versa. For arm64 it is `minimailboxes/all-in-one:latest-arm64` and `minimailboxes/all-in-one:beta-arm64`, respectively.
...
Additionally, there is a cronjob that runs once a day that checks for container and mastercontainer updates and sends a notification to all MiniMailboxes admins if a new update was found.

### How to easily log in to the AIO interface?
If your MiniMailboxes is running and you are logged in as admin in your MiniMailboxes, you can easily log in to the AIO interface by opening `https://yourdomain.tld/settings/admin/overview` which will show a button on top that enables you to log in to the AIO interface by just clicking on this button. **Note:** You can change the domain/ip-address/port of the button by simply stopping the containers, visiting the AIO interface from the correct and desired domain/ip-address/port and clicking once on `Start containers`.
...
1. Stop the mastercontainer with `sudo docker stop minimailboxes-aio-mastercontainer`
1. If the domaincheck container is still running, stop it with `sudo docker stop minimailboxes-aio-domaincheck`
...
1. Delete the docker network with `sudo docker network rm minimailboxes-aio`
1. Check which volumes are dangling with `sudo docker volume ls --filter "dangling=true"`
1. Now remove all these dangling volumes: `sudo docker volume prune` (on Windows you might need to remove some volumes afterwards manually with `docker volume rm minimailboxes_aio_backupdir`, `docker volume rm minimailboxes_aio_minimailboxes_datadir`). Also if you've configured `MINIMAILBOXES_DATADIR` to a path on your host instead of the default volume, you need to clean that up as well.
...
### Backup solution
MiniMailboxes AIO provides a local backup solution based on [BorgBackup](https://github.com/borgbackup/borg#what-is-borgbackup). These backups act as a local restore point in case the installation gets corrupted. 
...
2. If not already done, fire up the docker container and set up MiniMailboxes as per the guide.
...
Be aware that this solution does not back up files and folders that are mounted into MiniMailboxes using the external storage app.

Note that this implementation does not provide remote backups, for this you can use the [backup app](https://apps.minimailboxes.com/apps/backup).
...
# An example backup archive might be called 20220223_174237-minimailboxes-aio
# Then you can simply delete the archive with:
sudo borg delete --stats --progress "/mnt/backup/borg::20220223_174237-minimailboxes-aio"
...
if docker ps --format "{{.Names}}" | grep "^minimailboxes-aio-minimailboxes$"; then
    docker exec -it minimailboxes-aio-minimailboxes bash /notify.sh "Rsync backup successful!" "Synced the backup repository successfully."
else
...
One example for this would be `sudo docker exec -it -e DAILY_BACKUP=1 minimailboxes-aio-mastercontainer /daily-backup.sh`, which you can run via a cronjob or put it in a script.
...
### How to change the default location of MiniMailboxes's Datadir?
⚠️ **Attention:** It is very important to change the datadir **before** MiniMailboxes is installed/started the first time and not to change it afterwards! If you still want to do it afterwards, see [this](https://github.com/minimailboxes/all-in-one/discussions/890#discussioncomment-3089903) on how to do it.

You can configure the MiniMailboxes container to use a specific directory on your host as data directory. You can do so by adding the environmental variable `MINIMAILBOXES_DATADIR` to the initial startup of the mastercontainer. Allowed values for that variable are strings that start with `/` and are not equal to `/`.

- An example for Linux is `-e MINIMAILBOXES_DATADIR="/mnt/ncdata"`.
- On macOS it might be `-e MINIMAILBOXES_DATADIR="/var/minimailboxes-data"`
- For Synology it may be `-e MINIMAILBOXES_DATADIR="/volume1/docker/minimailboxes/data"`. 
- On Windows it must be `-e MINIMAILBOXES_DATADIR="minimailboxes_aio_minimailboxes_datadir"`. In order to use this, you need to create the `minimailboxes_aio_minimailboxes_datadir` volume beforehand:
    ```
    docker volume create ^
    --driver local ^
    --name minimailboxes_aio_minimailboxes_datadir ^
...
### How to allow the MiniMailboxes container to access directories on the host?
By default, the MiniMailboxes container is confined and cannot access directories on the host OS. You might want to change this when you are planning to use local external storage in MiniMailboxes to store some files outside the data directory and can do so by adding the environmental variable `MINIMAILBOXES_MOUNT` to the initial startup of the mastercontainer. Allowed values for that variable are strings that start with `/` and are not equal to `/`.

- Two examples for Linux are `-e MINIMAILBOXES_MOUNT="/mnt/"` and `-e MINIMAILBOXES_MOUNT="/media/"`.
- For Synology it may be `-e MINIMAILBOXES_MOUNT="/volume1/"`.
- On Windows is this option not supported.

After using this option, please make sure to apply the correct permissions to the directories that you want to use in MiniMailboxes. E.g. `sudo chown -R 33:0 /mnt/your-drive-mountpoint` and `sudo chmod -R 750 /mnt/your-drive-mountpoint` should make it work on Linux when you have used `-e MINIMAILBOXES_MOUNT="/mnt/"`. 
...
### How to adjust the upload limit for MiniMailboxes?
By default are uploads to MiniMailboxes limited to a max of 10G. You can adjust the upload limit by providing `-e MINIMAILBOXES_UPLOAD_LIMIT=10G` to the docker run command of the mastercontainer and customize the value to your fitting. It must start with a number and end with `G` e.g. `10G`.

### How to adjust the max execution time for MiniMailboxes?
By default are uploads to MiniMailboxes limited to a max of 3600s. You can adjust the upload time limit by providing `-e MINIMAILBOXES_MAX_TIME=3600` to the docker run command of the mastercontainer and customize the value to your fitting. It must be a number e.g. `3600`.
...
You can run AIO also with docker rootless. How to do this is documented here: [docker-rootless.md](https://github.com/minimailboxes/all-in-one/blob/main/docker-rootless.md)

### Huge docker logs
When your containers run for a few days without a restart, the container logs that you can view from the AIO interface can get really huge. You can limit the loge sizes by enabling logrotate for docker container logs. Feel free to enable this by following those instructions: https://sandro-keil.de/blog/logrotate-for-docker-container/

### Access/Edit MiniMailboxes files/folders manually
The files and folders that you add to MiniMailboxes are by default stored in the following directory: `/var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/` on the host. If needed, you can modify/add/delete files/folders there but **ATTENTION**: be very careful when doing so because you might corrupt your AIO installation! Best is to create a backup using the built-in backup solution before editing/changing files/folders in there because you will then be able to restore your instance to the backed up state.

After you are done modifying/adding/deleting files/folders, don't forget to apply the correct permissions by running: `sudo chown -R 33:0 /var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/*` and `sudo chmod -R 750 /var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/*` and rescan the files with `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ files:scan --all`.

### How to store the files/installation on a separate drive?
You can move the whole docker library and all its files including all MiniMailboxes AIO files and folders to a separate drive by first mounting the drive in the host OS (NTFS is not supported) and then following this tutorial: https://www.guguweb.com/2019/02/07/how-to-move-docker-data-directory-to-another-location-on-ubuntu/<br>
(Of course docker needs to be installed first for this to work.)

### How to edit MiniMailboxess config.php file with a texteditor?
You can edit MiniMailboxess config.php file directly from the host with your favorite text editor. E.g. like this: `sudo nano /var/lib/docker/volumes/minimailboxes_aio_minimailboxes/_data/config/config.php`. Make sure to not break the file though which might corrupt your MiniMailboxes instance otherwise. In best case, create a backup using the built-in backup solution before editing the file.

### Custom skeleton directory
If you want to define a custom skeleton directory, you can do so by putting your skeleton files into `/var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/skeleton/`, applying the correct permissions with `sudo chown -R 33:0 /var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/skeleton` and and `sudo chmod -R 750 /var/lib/docker/volumes/minimailboxes_aio_minimailboxes_data/_data/*` and setting the skeleton directory option with `sudo docker exec --user www-data -it minimailboxes-aio-minimailboxes php occ config:system:set skeletondirectory --value="/mnt/ncdata/skeleton"`. You can read further on this option here: [click here](https://docs.minimailboxes.com/server/stable/admin_manual/configuration_server/config_sample_php_parameters.html?highlight=skeletondir#:~:text=adding%20%3Fdirect%3D1-,'skeletondirectory',-%3D%3E%20'%2Fpath%2Fto%2Fminimailboxes)

### Fail2ban
You can configure your server to block certain ip-addresses using fail2ban as bruteforce protection. Here is how to set it up: https://docs.minimailboxes.com/server/stable/admin_manual/installation/harden_server.html#setup-fail2ban. The logpath of AIO is by default `/var/lib/docker/volumes/minimailboxes_aio_minimailboxes/_data/data/minimailboxes.log`. Do not forget to add `chain=DOCKER-USER` to your minimailboxes jail config (`minimailboxes.local`) otherwise the minimailboxes service running on docker will still be accessible even if the IP is banned. Also, you may change the blocked ports to cover all AIO ports: by default `80,443,8080,8443,3478` (see [this](https://github.com/minimailboxes/all-in-one#explanation-of-used-ports))

### LDAP
It is possible to connect to an existing LDAP server. You need to make sure that the LDAP server is reachable from the MiniMailboxes container. Then you can enable the LDAP app and configure LDAP in MiniMailboxes manually. If you don't have a LDAP server yet, recommended is to use this docker container: https://hub.docker.com/r/nitnelave/lldap. Make sure here as well that MiniMailboxes can talk to the LDAP server. The easiest way is by adding the LDAP docker container to the docker network `minimailboxes-aio`. Then you can connect to the LDAP container by its name from the MiniMailboxes container.
...
### USER_SQL
If you want to use the user_sql app, the easiest way is to create an additional database container and add it to the docker network `minimailboxes-aio`. Then the MiniMailboxes container should be able to talk to the database container using its name.

### phpMyAdmin, Adminer or pgAdmin
It is possible to install any of these to get a GUI for your AIO database. The pgAdmin container is recommended. You can get some docs on it here: https://www.pgadmin.org/docs/pgadmin4/latest/container_deployment.html. For the container to connect to the aio-database, you need to connect the container to the docker network `minimailboxes-aio` and use `minimailboxes-aio-database` as database host, `oc_minimailboxes` as database username and the password that you get when running `sudo grep dbpassword /var/lib/docker/volumes/minimailboxes_aio_minimailboxes/_data/config/config.php` as the password. 
...
### How to migrate from an already existing MiniMailboxes installation to MiniMailboxes AIO?
Please see the following documentation on this: [migration.md](https://github.com/minimailboxes/all-in-one/blob/main/migration.md)
...
1. New containers must be related to MiniMailboxes. Related means that there must be a feature in MiniMailboxes that gets added by adding this container.
...
3. The feature that gets added into MiniMailboxes by adding the container must be maintained by the MiniMailboxes GmbH. 
