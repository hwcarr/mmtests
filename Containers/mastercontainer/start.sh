...
if [ -n "$LOCAL_API_VERSION_NUMB" ] && [ -n "$API_VERSION_NUMB" ]; then
    if ! [ "$LOCAL_API_VERSION_NUMB" -ge "$API_VERSION_NUMB" ]; then
        echo "Docker API v$API_VERSION is not supported by your docker engine. Cannot proceed. Please upgrade your docker engine if you want to run MiniMailboxes AIO!"
        exit 1
    fi
else
...
if ! sudo -u www-data docker ps | grep -q "minimailboxes-aio-mastercontainer"; then
    echo "It seems like you did not give the mastercontainer the correct name?
Using a different name is not supported!"
    exit 1
elif ! sudo -u www-data docker volume ls | grep -q "minimailboxes_aio_mastercontainer"; then
...
if [ -n "$MINIMAILBOXES_DATADIR" ]; then
    if [ "$MINIMAILBOXES_DATADIR" = "minimailboxes_aio_minimailboxes_datadir" ]; then
        echo "MINIMAILBOXES_DATADIR is set to $MINIMAILBOXES_DATADIR"
    elif ! echo "$MINIMAILBOXES_DATADIR" | grep -q "^/" || [ "$MINIMAILBOXES_DATADIR" = "/" ]; then
        echo "You've set MINIMAILBOXES_DATADIR but not to an allowed value.
The string must start with '/' and must not be equal to '/'.
It is set to '$MINIMAILBOXES_DATADIR'."
        exit 1
    fi
fi
if [ -n "$MINIMAILBOXES_MOUNT" ]; then
    if ! echo "$MINIMAILBOXES_MOUNT" | grep -q "^/" || [ "$MINIMAILBOXES_MOUNT" = "/" ]; then
        echo "You've set NEXCLOUD_MOUNT but not to an allowed value.
The string must start with '/' and must not be equal to '/'.
It is set to '$MINIMAILBOXES_MOUNT'."
        exit 1
    elif [ "$MINIMAILBOXES_MOUNT" = "/mnt/ncdata" ] || echo "$MINIMAILBOXES_MOUNT" | grep -q "^/mnt/ncdata/"; then
        echo "'/mnt/ncdata' and '/mnt/ncdata/' are not allowed as values for MINIMAILBOXES_MOUNT."
        exit 1
    fi
fi
if [ -n "$MINIMAILBOXES_DATADIR" ] && [ -n "$MINIMAILBOXES_MOUNT" ]; then
    if [ "$MINIMAILBOXES_DATADIR" = "$MINIMAILBOXES_MOUNT" ]; then
        echo "MINIMAILBOXES_DATADIR and MINIMAILBOXES_MOUNT are not allowed to be equal."
        exit 1
    fi
fi
if [ -n "$MINIMAILBOXES_UPLOAD_LIMIT" ]; then
    if ! echo "$MINIMAILBOXES_UPLOAD_LIMIT" | grep -q '^[0-9]\+G$'; then
        echo "You've set MINIMAILBOXES_UPLOAD_LIMIT but not to an allowed value.
The string must start with a number and end with 'G'.
It is set to '$MINIMAILBOXES_UPLOAD_LIMIT'."
        exit 1
    fi
fi
if [ -n "$MINIMAILBOXES_MAX_TIME" ]; then
    if ! echo "$MINIMAILBOXES_MAX_TIME" | grep -q '^[0-9]\+$'; then
        echo "You've set MINIMAILBOXES_MAX_TIME but not to an allowed value.
The string must be a number. E.g. '3600'.
It is set to '$MINIMAILBOXES_MAX_TIME'."
        exit 1
    fi
fi
...
# Prevents issues like https://github.com/minimailboxes/all-in-one/discussions/565
curl https://minimailboxes.com &>/dev/null
if [ "$?" = 6 ]; then
    echo "Could not resolve the host minimailboxes.com."
...
# Don't allow access to the AIO interface from the MiniMailboxes container
# Probably more cosmetic than anything but at least an attempt
if ! grep -q '# minimailboxes-aio-block' /etc/apache2/apache2.conf; then
    cat << APACHE_CONF >> /etc/apache2/apache2.conf
# minimailboxes-aio-block-start
<Location />
order allow,deny
deny from minimailboxes-aio-minimailboxes.minimailboxes-aio
allow from all
</Location>
# minimailboxes-aio-block-end
APACHE_CONF
fi
...
if ! [ -f ./ssl.crt ] && ! [ -f ./ssl.key ]; then
    openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 -subj "/C=DE/ST=BE/L=Local/O=Dev/CN=minimailboxes.local" -keyout ./ssl.key -out ./ssl.crt
fi
if [ -f ./ssl.crt ] && [ -f ./ssl.key ]; then
    cd "$TMP_CERTS" || exit 1
    rm ./ssl.crt
...
print_green "Initial startup of MiniMailboxes All In One complete!
You should be able to open the MiniMailboxes AIO Interface now on port 8080 of this server!
E.g. https://internal.ip.of.this.server:8080

If your server has port 80 and 8443 open and you point a domain to your server, you can get a valid certificate automatically by opening the MiniMailboxes AIO Interface via:
