...
**Advice:** You may have a look at [this](https://github.com/minimailboxes/all-in-one/discussions/575#discussion-4055615) for a more complete example.
...
1. Add this to your Caddyfile:
    ...
    ```
    Of course you need to modify `<your-nc-domain>` to the domain on which you want to run MiniMailboxes. You also need to adjust `<provider>` and `<key>` to match your case. Also make sure to adjust the port 11000 to match the chosen APACHE_PORT. **Please note:** The above configuration will only work if your reverse proxy is running directly on the host that is running the docker daemon. If the reverse proxy is running in a docker container, you can use the `--network host` option (or `network_mode: host` for docker-compose) when starting the reverse proxy container in order to connect the reverse proxy container to the host network. If that is not an option for you, you can alternatively instead of `localhost` use the ip-address that is displayed after running the following command on the host OS: `ip a | grep "scope global" | head -1 | awk '{print $2}' | sed 's|/.*||'` (the command only works on Linux)
...
**Advice:** In order to make it work in your home network, you may add the internal ipv4-address of your reverse proxy as A DNS-record to your domain and disable the dns-rebind-protection in your router. Another way it to set up a local dns-server like a pi-hole and set up a custom dns-record for that domain that points to the internal ip-adddress of your reverse proxy (see https://github.com/minimailboxes/all-in-one#how-can-i-access-minimailboxes-locally). If both is not possible, you may add the domain to the hosts file which is needed then for any devices that shall use the server.
...
frontend LetsEncrypt_443
    ...
    # ACTION: MiniMailboxes
    use_backend MiniMailboxes if acl_60604e669c3ca4.13013327
...
# Backend: MiniMailboxes ()
backend MiniMailboxes
    mode http
    balance source
    server MiniMailboxes localhost:11000 
```
...
**Advice:** You may have a look at [this](https://github.com/minimailboxes/all-in-one/discussions/588#discussioncomment-2811152) for a more complete example.
...
Apart from that, there is this: [manual-install](https://github.com/minimailboxes/all-in-one/tree/main/manual-install)
...
Of course you need to modify `<your-nc-domain>` to the domain on which you want to run MiniMailboxes. Also change `<you>@<your-mail-provider-domain>` to a mail address of yours. Also make sure to adjust the port 11000 to match the chosen APACHE_PORT. **Please note:** The above configuration will only work if your reverse proxy is running directly on the host that is running the docker daemon. If the reverse proxy is running in a docker container, you can use the `--network host` option (or `network_mode: host` for docker-compose) when starting the reverse proxy container in order to connect the reverse proxy container to the host network. If that is not an option for you, you can alternatively instead of `localhost` use the ip-address that is displayed after running the following command on the host OS: `ip a | grep "scope global" | head -1 | awk '{print $2}' | sed 's|/.*||'` (the command only works on Linux)

**Advice:** You may have a look at [this](https://github.com/minimailboxes/all-in-one/discussions/588#discussioncomment-3040493) for a more complete example.
...
1. Add a `minimailboxes.toml` to the Treafik rules folder with the following content:
...
Of course you need to modify `<your-nc-domain>` in the minimailboxes.toml to the domain on which you want to run MiniMailboxes. Also make sure to adjust the port 11000 to match the chosen APACHE_PORT. **Please note:** The above configuration will only work if your reverse proxy is running directly on the host that is running the docker daemon. If the reverse proxy is running in a docker container, you can use the `--network host` option (or `network_mode: host` for docker-compose) when starting the reverse proxy container in order to connect the reverse proxy container to the host network. If that is not an option for you, you can alternatively instead of `localhost` use the ip-address that is displayed after running the following command on the host OS: `ip a | grep "scope global" | head -1 | awk '{print $2}' | sed 's|/.*||'` (the command only works on Linux)
...
On macOS see https://github.com/minimailboxes/all-in-one#how-to-run-aio-on-macos.
...
Simply translate the docker run command into a docker-compose file. You can have a look at [this file](https://github.com/minimailboxes/all-in-one/blob/main/docker-compose.yml) for some inspiration but you will need to modify it either way. You can find further examples here: https://github.com/minimailboxes/all-in-one/discussions/588
