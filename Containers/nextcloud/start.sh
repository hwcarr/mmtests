...
if [ -f "/var/www/html/config/config.php" ]; then
    ...
    # The code below is hopefully not needed anymore. Was introduced with https://github.com/minimailboxes/all-in-one/pull/218
...
sudo -u www-data touch "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file" &>/dev/null
if ! [ -f "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file" ]; then
    chown -R www-data:root "$MINIMAILBOXES_DATA_DIR"
    chmod 750 -R "$MINIMAILBOXES_DATA_DIR"
fi
sudo -u www-data rm -f "$MINIMAILBOXES_DATA_DIR/this-is-a-test-file"
